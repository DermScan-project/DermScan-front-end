...
// Now takes a messageId (not a raw Cloudinary url) — fetches a fresh signed URL
// from our own backend first, then downloads through that.
...
interface LightboxState {
  messageId: string;
  url: string; // still needed to actually display the image in the lightbox
  name: string;
}
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    socket.on("new_message", (msg: Message) => {
      ...
      loadConversations();
      if (currentSelectedId && (msg.senderId === currentSelectedId || msg.receiverId === currentSelectedId)) {
        setMessages((prev) => {
  ...
  // Now takes messageId instead of a raw url
  const handleFileContextMenu = (e: React.MouseEvent, messageId: string, filename: string) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const openConversation = useCallback(
    async (medecinId: string, medecinInfo?: MedecinContact) => {
      ...
      try {
        ...
        setConversations((prev) => prev.map((c) => (c.medecin.id === medecinId ? { ...c, nonLus: 0 } : c)));
        setTotalUnread((prev) =>
          Math.max(0, prev - (conversations.find((c) => c.medecin.id === medecinId)?.nonLus || 0))
        );
      } finally {
  ...
  .map((m) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left transition-colors ${
                    isSelected ? "bg-sauge-clair border border-sauge/20" : "hover:bg-sauge-clair/30 border border-transparent"
                  }`}
                >
                  ...
                  {hasConv && (
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#1B3A2D" strokeWidth="2" className="shrink-0 opacity-40">
          ...
          {!selectedId ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-8">
              <div className="w-14 h-14 rounded-2xl bg-sauge-clair flex items-center justify-center">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#1B3A2D" strokeWidth="1.5">
                  <path d="M21 11.5a8.4 8.4 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.4 8.4 0 01-3.8-.9L3 21l1.9-5.7a8.4 8.4 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.4 8.4 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <p className="text-sm font-medium text-encre">Sélectionnez une conversation</p>
              <p className="text-xs text-ardoise/50">Choisissez un praticien actif à gauche pour démarrer</p>
            </div>
          ) : (
            <>
              <div className="px-5 py-3 border-b border-ardoise/10 bg-white flex items-center gap-3 shrink-0">
                ...
                <div>
                  <p className="text-sm font-semibold text-encre">Dr. {selectedMedecin?.nomComplet}</p>
              ...
              <div className="flex-1 overflow-y-auto min-h-0 px-5 py-4 flex flex-col gap-3">
                {loadingMsgs && (
                  <div className="flex flex-col gap-3">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className={`h-10 w-48 rounded-2xl bg-ardoise/8 animate-pulse ${i % 2 === 0 ? "self-end" : "self-start"}`} />
                    ))}
                  </div>
                )}
                      ...
                      <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                        <div
                          className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                            isMine ? "bg-encre text-white rounded-br-md" : "bg-white border border-ardoise/10 text-encre rounded-bl-md"
                          }`}
                        >
                          ...
                          {msg.fichierUrl && isImage && (
                            <img
                              ...
                              onClick={() =>
                                setLightboxImage({ messageId: msg.id, url: msg.fichierUrl!, name: msg.fichierNom || "image" })
                              }
                              onContextMenu={(e) => handleFileContextMenu(e, msg.id, msg.fichierNom || "image")}
                            />
                          )}

                          {msg.fichierUrl && !isImage && (
                            <button
                              onClick={() => downloadFile(msg.id, msg.fichierNom || "fichier")}
                              className="flex items-center gap-1.5 text-xs underline opacity-80 mt-1"
                              onContextMenu={(e) => handleFileContextMenu(e, msg.id, msg.fichierNom || "Pièce jointe")}
                            >
                          ...
                          <p className={`text-[10px] mt-1 ${isMine ? "text-white/50" : "text-ardoise/40"}`}>{timeAgo(msg.createdAt)}</p>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="px-4 py-3 border-t border-ardoise/10 bg-white shrink-0">
                {selectedFile && (
                  <div className="mb-2 flex items-center gap-2 px-3 py-2 rounded-xl bg-sauge-clair/40 border border-sauge/20">
                    {filePreview ? (
                      <img src={filePreview} alt="" className="w-10 h-10 rounded-lg object-cover shrink-0" />
                    ) : (
                ...
                <p className="text-[10px] text-ardoise/30 mt-1.5 pl-1">Entrée pour envoyer · Maj+Entrée pour sauter une ligne</p>
              </div>
            </>
          )}
        </div>
      ...
      {lightboxImage && (
        <div
          ...
          onContextMenu={(e) => handleFileContextMenu(e, lightboxImage.messageId, lightboxImage.name)}
        >
          <button
            onClick={(e) => {
              e.stopPropagation();
          ...
          <img src={lightboxImage.url} alt={lightboxImage.name} className="max-w-full max-h-full rounded-lg object-contain" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
    </div>
  );
